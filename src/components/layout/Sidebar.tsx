...
import { getUnreadUpdatesCountForStudent, getUnreadMessagesCountForCompany } from '../../services/ticketService';
...
const Sidebar = ({ userRole }: SidebarProps) => {
  ...
  useEffect(() => {
    const fetchTicketNotifications = async () => {
      ...
      try {
        if (user.role === UserRole.STUDENT) {
          count = await getUnreadUpdatesCountForStudent(user.id);
        } else if (user.role === UserRole.COMPANY_ADMIN && user.companyId) {
          count = await getUnreadMessagesCountForCompany(user.companyId);
        }
        setTicketNotifCount(count);
      } catch (error) {
    ...
    const interval = setInterval(fetchTicketNotifications, 60000); // Refresh every minute
    ...
    switch (userRole) {
      ...
      case UserRole.COMPANY_ADMIN:
        return [
          ...
          {
           name: 'Bibliothèque',
           path: '/company-admin/library',
           icon: Briefcase,
           description: 'Gérer les ressources'
          },
          {
           name: 'Tickets',
           path: '/company-admin/tickets',
           icon: HelpCircle,
      ...
      case UserRole.SUPER_ADMIN:
        return [
          ...
          },
          {
           ...
           icon: Briefcase,
           description: 'Gérer la bibliothèque'
          },
          {
           name: 'Tickets',
  ...
  const navItems = getNavItems(ticketNotifCount);
