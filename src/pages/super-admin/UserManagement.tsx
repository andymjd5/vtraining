...
const UserManagement = () => {
  ...
  const filteredUsers = users.filter(user => {
    ...
    // Filtrage par onglet (garde la logique existante)
    let matchesTab = true;
    if (selectedTab !== 'all') {
    ...
    return matchesSearch && matchesCompany && matchesRole && matchesStatus && matchesTab;
  });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="space-y-4">
          {/* Message d'invitation si aucun filtre appliqué */}
          {!hasAppliedFilters && (
            <div className="text-center py-12">
              <Filter className="mx-auto h-12 w-12 text-gray-400" />
              <h3 className="mt-2 text-sm font-medium text-gray-900">Sélectionnez vos critères de filtrage</h3>
              <p className="mt-1 text-sm text-gray-500">
                Utilisez les filtres ci-dessus pour afficher les utilisateurs selon vos critères.
              </p>
              <Button 
                className="mt-4"
          ...
          {/* Contenu existant - affiché seulement si des filtres sont appliqués */}
          {hasAppliedFilters && (
            <>
              {/* Filters and Search */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex space-x-2">
                  ...
                  >
                    Tous ({getFilteredCount()})
                  </button>
                  {Object.values(UserRole).map((role) => {
                    const roleCount = getFilteredCount(role);
